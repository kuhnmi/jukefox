...
public class GLView extends SurfaceView implements SurfaceHolder.Callback {
	...
	/**
	 * A generic GL JoinableThread. Takes care of initializing EGL and GL.
	 * Delegates to a Renderer instance to do the actual drawing.
	 * 
	 */

	class GLThread extends JoinableThread {
		...
		private boolean needToWait() {
			// Log.v(TAG, "" + mPaused + " " + (! mHasFocus) + " "+ (!
			// mHasSurface) + " "+ mContextLost + " ");
			// return (mPaused || (! mHasFocus) || (! mHasSurface) ||
			// mContextLost)
			return (mPaused || !mHasSurface || mContextLost) && !mDone;
		}
		...
		public void requestExitAndWait() {
			...
			try {
				...
				JoinableThread.currentThread().interrupt();
			}
		}
